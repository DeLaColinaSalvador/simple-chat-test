...
class MyCoolAgent extends Agent {
  constructor(conf) {
    ...
    this.on(
      ...
      this.onMessagingNotification.bind(this)
    );
  ...
  onConnected(msg) {
    ...
    this.subscribeExConversations(
      ...
      (e, resp) =>
        console.log("subscribeExConversations", this.conf.id || "", resp || e)
    );
    ...
    notificationBody.changes.forEach((change) => {
      if (change.type === "UPSERT") {
        ...
        // an existing conversation
          ...
          if (
            change.result.conversationDetails.participants.filter(
              (p) => p.role === "CONSUMER"
            )[0].id !== conversation.consumerId
          ) {
    ...
    this.getUserProfile(conversation.consumerId, (e, profileResp) => {
      ...
      this.publishEvent({
        ...
        event: {
          type: "ContentEvent",
          contentType: "text/plain",
          message: `Consumer stepped up in conversation with ${JSON.stringify(
            profileResp
          )}`,
        },
      });
