...
class SDK extends Events {
  ...
  _login(conf, domains, callback) {
    ...
    if (!domains.agentVep) {
      return callback(
        new SDKError("could not fetch domains", null, SERVICES.DOMAIN_NAMES)
      );
    }
  ...
  _createAndSendRequest(
    ...
    if (Transformer[type]) {
      ...
      ({ body, type, headers, metadata, encodedMetadata } = Transformer[type](
        { body, type, headers, metadata, encodedMetadata },
        this
      ));
    }
  ...
  _checkForErrors() {
    ...
    if (timedOutCallbacks.length) {
      for (let i = 0; i < timedOutCallbacks.length; i++) {
        ...
        this._notifyOutCome(
          timedOutCallbacks[i],
          new SDKError("Request has timed out", null, SERVICES.MESSAGING),
          true
        );
      }
    }
